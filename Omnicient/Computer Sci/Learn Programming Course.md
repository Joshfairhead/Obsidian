...
Tags: []
---
Related: [[Computer Science]] [[JavaScript]] [[Learn Javascript Course]]
# Learn Programming Course

## Learn Programming Notes
[Link](https://learnprogramming.online/app.html)
[Start](https://learnprogramming.online/app.html?from_completed=1)
...
sam.canVote(); // true
```
- The methods `.getFullName()`, `.getInitials()` and `.canVote()` in the last example are **instance methods** because we call them on the `sam` variable (which is an instance of the class `User`).
